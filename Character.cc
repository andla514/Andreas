...
void Character::move_player()
{
	...
	else if (col_inc != 0 && row_inc == 0 && our_map->can_move_to(row, col + col_inc))
    {
		col += col_inc;
		is_moving = true;
		my_timer.restart();
    ...
    else if (our_map->can_move_to(row + row_inc, col) && row_inc != 0)
    {
		row += row_inc;
		is_moving = true;
		my_timer.restart();
    }
    else if (our_map->can_move_to(row, col + col_inc) && col_inc != 0)
    {
		col += col_inc;
		is_moving = true;
		my_timer.restart();
