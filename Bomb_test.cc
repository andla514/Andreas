...
TEST_CASE("Bomb test")
{
    ...
    SECTION("Detonate")
    {
	G->add_bomb(1, 1, make_unique<Bomb>(Bomb{1, 1, settings, G, 1}));
	G->set_element(1, 1, 3);
	CHECK(G->get_element(1, 1) == 3);
	G->add_explosion(2, 1, make_unique<Explosion>(Explosion(2, 1, 3, false, G)));
...
TEST_CASE("Uppdate")
{
    ...
    G->add_bomb(1, 2, make_unique<Bomb>(Bomb{1, 2, s2, G, 2}));
    G->set_element(1, 2, 3);
...
TEST_CASE("Detonate bomb from explotion")
{
    ...
    G->add_bomb(1, 1, make_unique<Bomb>(Bomb{1, 1, s, G, 2}));    
	G->set_element(1, 1, 3);
    G->add_bomb(1, 2, make_unique<Bomb>(Bomb{1, 2, s, G, 2}));
	G->set_element(1, 2, 3);
    (G->get_bomb_reference(1, 1)).detonate();
...
TEST_CASE("Detonate and destroy item")
{
	...
	G->add_bomb(1, 1, make_unique<Bomb>(Bomb{1, 1, s, G, 2}));    
	G->set_element(1, 1, 3);
	G->add_item(1, 2, make_unique<Item_Inc_Bombs>(1, 2));
	G->set_element(1, 2, 2);
	(G->get_bomb_reference(1, 1)).detonate();
	CHECK_THROWS(G->get_item_reference(1, 2));
}